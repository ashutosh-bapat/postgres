...
{
	if (!root->rinfo_hash)
		root->rinfo_hash = rinfohash_create(root->planner_cxt, 1000, NULL);
...
{
	rinfohash_hash *rinfo_tab = get_restrictinfo_table(root);
	rinfo_tab_key key;
	...
	/*
	 * If the given RestrictInfo is already present in the hash table, multiple
	 * instances of the same RestrictInfo may have been created. This function is
	 * a good place to flag that. While it does not affect the correctness, it
	 * defeats the purpose of maintaining the hash table.
	 */
	Assert(!found);

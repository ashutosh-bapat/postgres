					...
					JoinDomain *jdomain)
{
	...
	foreach(lc1, root->eq_classes)
	{
		...
		foreach(lc2, cur_ec->ec_members)
		{
			 ...
			 * Child derived clauses are stored outside an EquivalenceClass, but
			 * they don't need to worry about them here.
			*/
									   ...
									   EquivalenceClass *ec)
{
	...
	foreach(lc, ec->ec_members)
	{
		 ...
		 * clauses are stored outside EquivalenceClass. But we don't bother about
				   ...
				   EquivalenceClass *parent_ec)
{
	 ...
	 * function. Hence allocate the memory for them in planner context.
	 * Necessary for GEQO.
	 */
	oldcontext = MemoryContextSwitchTo(root->planner_cxt);
	...
	if (leftem->em_is_child || rightem->em_is_child)
	{
		...
		{
			/*
			 * Make sure that we got the child clause made from given equivalence
			 * members which are part of the given equivalence class.
			 */
	...
	/*
	 * If it's not a child RestrictInfo save it for possible re-use. Child
	 * RestrictInfos will be stored separately later.
	 */
	if (!parent_rinfo)
		ec->ec_derives = lappend(ec->ec_derives, rinfo);
	...
	if (parent_rinfo)
	{
		...
		/*
		 * When merging equivalence classes we merge the clauses derived from
		 * ECs. But by the time we derive child clauses, EC merging should be
		 * complete and hence it should be safe not to save child clauses in
		 * derived list. If that's not the case we might miss merging child
		 * derived clauses as well.
		 */
								  ...
								  EquivalenceMember *em)
{
	...
	/*
	 * The child derived clauses are stored outside an EquivalenceClass. If this
	 * function is passed a child EC member, we have to find its parent EC
	 * member, find the corresponding parent clause and then find the child
	 * clause. But as of now this function does not receive child EC members, so
	 * we don't worry about all that.  Following Assert will let us know if
	 * things change.
	 */

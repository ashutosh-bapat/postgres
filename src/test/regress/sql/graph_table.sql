...
-- l1 is shared by all vertex tables and edge tables.  A view using it prevents
-- the label from being dropped from any of those tables.
CREATE VIEW v_shared_label AS SELECT * FROM GRAPH_TABLE (g1 MATCH (v IS l1) COLUMNS (v.elname));
ALTER PROPERTY GRAPH g1 ALTER VERTEX TABLE v1 DROP LABEL l1;  -- error

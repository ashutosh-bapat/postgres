...
--
-- Dropping property graph components implicitly or explicitly with CASCADE.
--
-- The output of the information schema queries below are used to verify the
...
-- because it remains associated with t3l1.
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t3 DROP LABEL t3l2;
-- Cascaded drop should clean up the orphaned labels and properties and objects
-- dependent on them.
...
\df orphan_f
-- Dropping a property should (even in CASCADE mode) should not leave the
-- property graph in an inconsistent state.
...
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t1 ALTER LABEL shared_l
    DROP PROPERTIES (shared_p);  -- error
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t1 ALTER LABEL shared_l
    DROP PROPERTIES (shared_p) CASCADE;  -- error
DROP FUNCTION shared_f(); -- error
DROP FUNCTION shared_f() CASCADE; -- error
-- drop the label since it doesn't contain anything interesting for the
-- following tests.
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t1 DROP LABEL shared_l;
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t2 DROP LABEL shared_l;

-- A view using a property through a specific label should prevent that
-- label-property association from being dropped, even if the property remains
-- elsewhere in the graph.
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t2 ADD LABEL view_l
    PROPERTIES (i AS x);
CREATE VIEW g4_view AS
        SELECT * FROM GRAPH_TABLE (g4 MATCH (v IS view_l) COLUMNS (v.x));
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t2
    ALTER LABEL view_l DROP PROPERTIES (x);  -- error
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t2
    ALTER LABEL view_l DROP PROPERTIES (x) CASCADE;
\d g4_view
-- drop the label, it does not have any interesting characteristics for the
-- following tests.
ALTER PROPERTY GRAPH g4 ALTER VERTEX TABLE t2 DROP LABEL view_l;

	...
	for (rti = 1; rti <= list_length(plannedstmt->rtable); ++rti)
	{
		...
		switch (rte->rtekind)
		{
			...
			case RTE_GRAPH_TABLE:
				/*
				 * We should not see RTE of this kind here since property graph
				 * RTE gets converted to subquery RTE in RewriteGraphTable().
				 * In case we decide not to do the conversion and leave RTEkind
				 * unchanged in future, print correct name of RTE kind.
				 */
				kind = "property_graph";
				break;
		}
		 ...
		 * fields, so skip them. No graph table specific fields are required to
		 * be printed.
		 */
